...
def station_stats(df):
    ...
    print(f'Most common end station: {
          common_end_station}. Count: {common_end_station_count}')

    # TO DO: display most frequent combination of start station and end station trip
    start_end_counts = (df['Start Station'] + ' - ' +
...
def user_stats(df):
    ...
    if 'Birth Year' in df.columns:
        ...
        print(f'\tMost common birth year: {
...
def show_individual_data(df):
    ...
    for i in range(0, len(df), 5):
        ...
        see_more = input(
            "Want to see 5 more records? (yes/no).\n").lower()
...
def main():
    while True:
        ...
        show_records = input(
            "Would you like to view individual trip data? (yes/no).\n")
